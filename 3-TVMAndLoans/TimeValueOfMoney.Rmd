...
$$F = 26(1+.02)^{500} = `r round(26*(1+.02)^500,2)`$$
...
$$P = \frac{100K}{(1+.1)^{10}} = `r round(100/1.1^10,2)`K$$
...
![https://en.wikipedia.org/wiki/Actuarial_notation](Actuarial_notation.png){width=20% }
...
![](sensit-sensitivity-analysis-spider-chart.png)
...
+ Basic Present Worth Examples for Common Shapes
    + $P = 3 (P|A, i= 10\%, 5 )$
    + $P = 2 (P|G, i= 10\%, 4 )$
    + $P = 10 (P|A_1, i= 10\%, g = 100\%, 10 )$
+ Hint, if you know $(P|X)$,  $(F|X) = (P|X) (1+r)^N$

## Step Through For Basics

$$Looking~For = Number(Looking~For|What~is~number, i, How ~Long)$$

+ $P = How~tall(P|A, i, How~Long)$
+ $P = Change~From~Last(P|G, i, Non-zero + One)$
+ $P = Time~1~Value(P|A_1, i, Growth~Rate, How~Long)$
...
```
  ...
  + Tabular: $10 * `r round(pgivena(1, .1, 5),4)`$
  + Closed form: $10(P|A, i = 10\%, 5) = 10 \frac{( 1+.1)^5 - 1}{.1 (1+.1)^5}$ 
  + Either way you should get `r round(pgivena(10, .1, 5),2)`.  
...
+ Tables have limited digits.  Don't use them for large values
