...
print(ggplot(cf,aes(x,y,label=y))+geom_bar(stat="Identity",color='gray',fill='gray')+xlab("Period")+ylab("Cash Flow")+geom_text())
```
...
## Just to be clear about factor notation
...
cf$LG <- c(0,0,1,2,3,4)
cf$C <- c(0,4,4,4,4,4)
print(ggplot(cf,aes(x,C,label=C))+geom_bar(stat="Identity",color='gray',fill='gray')+xlab("Period")+ylab("Cash Flow")+geom_text())

print(ggplot(cf,aes(x,LG,label=LG))+geom_bar(stat="Identity",color='gray',fill='gray')+xlab("Period")+ylab("Cash Flow")+geom_text())
```
...
+ See the linear gradient that has G of 1 and is 5 long $1(P|A,i,5)$
...
$$ \frac{1(P|A_1, i, g = 100\%, 4)}{(1+i)^{-1}} + \frac{16 (P|A, i, 6)}{(1+i)^3} - \frac{2 (P|G, i, 6)}{(1+i)^3}$$
...
+ The effective interest rate per payment period is $\frac{.06}{12}= .05\%$

+ Payment on amortizing loan is $200000 (A|P, i = .05\%, 360) = `r agivenp(200000,.005,360)`$.
